<template>
  ...
  <Teleport to="body">
    <div
      class="modal-wrapper"
      :class="{ 'is-visible': showDeleteModal }"
      :aria-hidden="!showDeleteModal"
    >
      <div class="modal-background" @click="showDeleteModal = false" />
      <div
        class="modal food-delete-modal"
        role="alertdialog"
        aria-modal="true"
...
onMounted(async () => {
  ...
  food.value = await db.food_items.get(foodId.value) ?? null
  if (food.value) {
    form.name              = food.value.name
    form.brand             = food.value.brand ?? ''
    form.barcode           = food.value.barcode ?? ''
    form.calories_per_100g = food.value.calories_per_100g
    form.protein_per_100g  = food.value.protein_per_100g
    form.carbs_per_100g    = food.value.carbs_per_100g
    form.fat_per_100g      = food.value.fat_per_100g
    form.fiber_per_100g    = food.value.fiber_per_100g ?? null
    form.sugar_per_100g    = food.value.sugar_per_100g ?? null
  }
...
async function handleSave() {
  ...
  isSaving.value = true
  try {
    await foodStore.updateItem(foodId.value, {
      name:              form.name,
...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape' && showDeleteModal.value) showDeleteModal.value = false
}

async function handleDelete() {
  isDeleting.value = true
  try {
    await foodStore.deleteItem(foodId.value)
...
.food-delete-modal__icon-wrap {
  margin-bottom: calc(#{$spacing} * 0.75);
}

.food-delete-modal__icon {
  color: var(--error);
}
